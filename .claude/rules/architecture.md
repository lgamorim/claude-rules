...
- Repo/solution layout: source under `src/<ProjectName>/`, tests under `test/<ProjectName>.Tests/` (singular `test`), one solution file (`.sln`/`.slnx`) per repo or example, at its root, referencing every project beneath it.
